...
var SolidusServer = function( options ){
  ...
  if (options.start_server) {
    ...
    handlebars.partialsDir.push({templates: handlebars.getTemplates(paths.extra_partials)});
  }
  ...
  // partials/something             -> /.../site/views/partials/something.hbs
  // some-module/partials/something -> /.../site/node_modules/some-module/partials/something.hbs
  this.viewPath = function(view_name) {
    // Check if a local site view exists, else return the extra partials path
    var view_path = path.join(this.paths.views, view_name + '.' + DEFAULT_VIEW_EXTENSION);
    if (!this.views[view_path] && !fs.existsSync(view_path)) {
      view_path = path.join(this.paths.extra_partials, view_name + '.' + DEFAULT_VIEW_EXTENSION);
    }
    return view_path;
  };
