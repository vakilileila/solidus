...
var SolidusServer = function( options ){
	...
	var paths = this.paths = {
		...
		resources: path.join( options.site_path, 'resources.json' ),
	...
	this.setupResources = function(){
		...
		try {
			delete require.cache[require.resolve( paths.resources )];
			var resources_config = require( paths.resources );
			var resources = this.resources = resources_config.resources;
			// extend resources with matching options
			if( resources_config.options ){
				_.each( resources, function( resource, name ){
					var url = resource.url || resource;
					_.each( resources_config.options, function( options, match ){
						var matcher = new RegExp( match, 'ig' );
						if( matcher.test( url ) ){
							resources[name] = _.extend( resource, options );
						}
					});
				});
			}
	...
	this.watch = function(){
		...
		var resources_regex = new RegExp( paths.resources, 'i' );
		...
		watcher.on( 'add', function( file_path ){
			...
			else if( resources_regex.test( file_path ) ) solidus_server.setupResources();
		});
		watcher.on( 'unlink', function( file_path ){
			...
			else if( resources_regex.test( file_path ) ) solidus_server.clearResources();
		});
