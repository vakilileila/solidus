...
var SolidusServer = function( options ){
  ...
  this.updatePreprocessor = function(path) {
  ...
  this.setupPreprocessors = function() {
    ...
    try {
      ...
      var preprocessors = require(paths.preprocessors_config);

      for (var view in preprocessors) {
        var preprocessor = preprocessors[view]();
        var resources = this.setupResources(preprocessor.resources);
        preprocessors[view] = new Preprocessor(preprocessor.process, {view: view, resources: resources, server: solidus_server});
      }
  ...
  this.setupResources = function(resources) {
    for (var name in resources) {
      // reassign string contents to url property
      if (_.isString(resources[name])) {
        resources[name] = {url: resources[name]};
      }

      // mixin global resource options
      if (this.auth) {
        _.each(this.auth, function(options, match) {
          var matcher = new RegExp(match, 'ig');
          if (matcher.test(resources[name].url)) {
            resources[name] = _.extend(resources[name], options);
          }
        });
      }
    }
  ...
  this.watch = function(){
    ...
    var preprocessor_config_regex = new RegExp( escapePathForRegex( paths.preprocessors_config ), 'i' );
    ...
    watcher.on( 'add', function( file_path ){
      ...
      else if( preprocessor_config_regex.test( file_path ) ) solidus_server.setupPreprocessors();
      else if( redirects_regex.test( file_path ) ) solidus_server.setupRedirects();
      else if( auth_regex.test( file_path ) ) {
        solidus_server.setupAuth();
        solidus_server.setupPreprocessors();
      }
    });
