...
var Page = function( page_path, options ){
	...
	this.parseConfig = function( callback ){
		...
		var global_resources = server.resources_config.resources;
		var global_resources_options = server.resources_config.options;
			...
			try {
				...
				if( params.resources ){
					for( var name in params.resources ){
						if( !isURL( params.resources[name] ) && _.isString( params.resources[name] ) ){
							var global_resource = global_resources[name];
							if( global_resource ){
								params.resources[name] = global_resource;
							}
							else {
								server.logger.log( 'No global resource "'+ name +'"', 3 );
					...
					// extend resources with matching options
					if( global_resources_options ){
						_.each( params.resources, function( resource, name ){
							var url = resource.url || resource;
							_.each( global_resources_options, function( options, match ){
								var matcher = new RegExp( match, 'ig' );
								if( matcher.test( url ) ){
									params.resources[name] = _.extend( resource, options );
								}
							});
						});
					}
	...
	this.fetchResource = function( resource_data, params, callback ){
		...
		resource_data.url = resource_data.url.replace( /\{([^\}]*)\}/ig, function( match, capture, index, str ){
			var replacement = params[capture] || '';
			return replacement;
		});
		// attempt to get resource from cache
		var cached_resource = cache.get( resource_data.url );
		if( cached_resource ) return callback( null, cached_resource );
		// fetch resource remotely
		var req = hyperquest( resource_data.url, resource_data, function( err, res ){
	...
	this.render = function( req, res, options ){
		...
		setTimeout( renderPage.bind( this, context ), DEFAULT_PAGE_TIMEOUT );
